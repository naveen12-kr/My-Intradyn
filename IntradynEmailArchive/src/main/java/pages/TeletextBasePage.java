...
public class TeletextBasePage {
	...
	public static void openUrl() {
		driver.get(EnvironmentUtils.getConfig().getProperty("teletext_url"));
		waitForPageLoad(10);
	}
	...
	public static boolean isElementEnabled(By ele, String selector) {
		...
		try {
			...
			System.out.println("INFO: Element " + selector + " is Enabled on screen ");
			return status;
	...
	public static void clickElement1(By ele, String selector) {
		WebElement wEle = findObject1(ele, selector);
		try {
			Actions act = new Actions(driver);
			act.click(wEle).build().perform();
