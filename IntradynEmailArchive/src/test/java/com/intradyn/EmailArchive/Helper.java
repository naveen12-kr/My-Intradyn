...
import com.intradyn.Runner.Test.IntradynRegressionRunner;
...
public class Helper extends BasePage {
	...
	public void closeSetup(Scenario scenario) throws InvalidFormatException, IOException {
			...
			ExcelOperation.updateTestStatus(scenario.getStatus(),scenario.getName());
		...
		if (scenario.isFailed()) {
			try {
				...
				File file = new File(
						path+"/target/screenshot_failure/screenshot_" + scenario.getName() + "_" + randomString + ".png");
				file.getParentFile().mkdirs();
				FileOutputStream out = new FileOutputStream(file);
				out.write(screenshot);

				out.close();
				Reporter.addScreenCaptureFromPath("../../target/screenshot_failure/screenshot_" + scenario.getName()+ "_" + randomString + ".png");
